...
1. **YAML Header** surrounded by --- on top and bottom; yet another markdown language
...
A handy cheat sheet for R markdown can be found - use in the challenge today [here](https://www.rstudio.com/wp-content/uploads/2015/03/rmarkdown-reference.pdf). Another one can be found [here](https://www.rstudio.com/wp-content/uploads/2015/02/rmarkdown-cheatsheet.pdf).
...
* Store *code*, *visulaizations*, and *text* in one place
...
can't run it, just shows up knitted only
...
### Installing packages
Create an R chunk below that installs the package `knitr`. Instead of commenting out the code, customize the chunk options such that the code is not evaluated (i.e., not run). echo is print if true?
...
```{r Setup, message=TRUE, msg=FALSE}
getwd()
library('tidyverse')
library('knitr')
...
```{r, echo=FALSE}
NTL.Nutrients.raw <- read.csv("./Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv")
