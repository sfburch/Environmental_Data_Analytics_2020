...
#setwd("/Users/katerisalk/Documents/Duke/Courses/Environmental_Data_Analytics") #absolute file path could be a problem bc we wouldn't have Kateri's docs on our computer, so you have to have # in front of setwd
```
...
```{r, message = FALSE} 
# Load package ^the above msg can be useful to not include the below (in case of aski text) when trying to knit to pdf
library(tidyverse)
```
...
USGS.flow.data <- read.csv("./Data/Raw/USGS_Site02085000_Flow_Raw.csv") #starting with dot means going one file in, holding down alt over csv file allows you to copy it as a pathname, shift works too; two dots signals go out one folder rather than in one folder

# What happens if we don't assign a name to our imported dataset?
#read.csv("./Data/Raw/USGS_Site02085000_Flow_Raw.csv") #have to call it something to show up in environment

# Another option is to choose with your browser, comment which file you're actually loading
...
> ANSWER: You can find this file w/in the metadata folder w/in the data folder. This README file allows the individual to gather descriptions, dates, file formats, and traceability overall. Just the CSV alone is not helpful enough.
...
# Rename columns (from the metadata) run 92 and then 90 again and you get different things (why?)
...
class(USGS.flow.data$datetime) #asking for specific columns (just run what is in parentheses)
...
```{r}
summary(USGS.flow.data$discharge.mean) #these both tell you how many NAs there are in dataset 
...
> Answer: remove: only want to analyze presence data vs presence/absence data and if you want to compare or correlation matrix across full data set; retain: allows you to know where you are lacking site data

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #fewer rows by omitting NAs - could be dangerous
dim(USGS.flow.data)
dim(USGS.flow.data.complete)

mean(USGS.flow.data.complete$discharge.mean)
sd(USGS.flow.data.complete$discharge.mean)
summary(USGS.flow.data.complete$discharge.mean) #would need to code SD separately
...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") #we have to tell r it is a date and which format; format is whatever format the data is in, not what R is in
```
...
```{r}
write.csv(USGS.flow.data, file = "./Data/Processed/USGS_Site02085000_Flow_Processed.csv", row.names=FALSE) #review!
