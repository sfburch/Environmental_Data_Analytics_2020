...
### One-way ANOVA
A one-way ANOVA is the same test in practice as a two-sample t-test but for three or more groups. In R, we can  run the model with the function `lm` or `aov`, the latter of which which will allow us to run post-hoc tests to determine pairwise differences. Stands for analysis of variance. We're working with y = mx + b. y=Betax + alpha + E (epsilon is error); we don't have slopes, we're comparing a bunch of means (alpha values)

Does total litter biomass differ among sites at the Niwot Ridge LTER?
```{r}
# Wrangle the data
Litter.Totals <- Litter %>% #is this if/then? piping
  group_by(plotID, collectDate, nlcdClass) %>% #nlcdclass to be used for later purposes
  summarise(dryMass = sum(dryMass)) #will tkae everything that has group by above and then sum all of it by, will add up all the dry masses for each category and will do it for each set
...
# ANOVA is robust against departures from equal variance. flag that what doesn't work??
bartlett.test(Litter.Totals$dryMass ~ Litter.Totals$plotID)

# Format ANOVA as aov
Litter.Totals.anova <- aov(data = Litter.Totals, dryMass ~ plotID)
summary(Litter.Totals.anova) #dry mass is y, the relationship we're testing

# Format ANOVA as lm; will start at 15.6), run both aov and lm objects
...
# Extract groupings for pairwise relationships, if the letters match they are not statistcially significant values between those groups
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE)
Litter.Totals.groups

# Graph the results, if the letters match they are not statistcially significant values between those groups, 45 deg angle will help read the bottom on the x-axis, adjust vertically so it doesn't plot on top maximum value, set size of letters, you cna also assign labels has the be in exact order as data set (alphabetically); abc matches the first in the date set (which is 040=abc); abc is max value for that option, i.e. for 046 there were no significant outliers so it shows up right above the box and whisker plot
Litter.Totals.plot <- ggplot(Litter.Totals, aes(x = plotID, y = dryMass)) +
  ...
  ylim(0, 85)
print(Litter.Totals.plot) #total litter biomass differed signifacntly among sites at Niwot Ridge LTR (figure 1; ANOVA, F11,198 = 4.813, p < <0.00021)?? WHAT; 4.813 is the test statistic, not the p-value, p val is meaningless without this test value; p value is computed from this test statistic
```
...
# dunnTest(Litter.Totals$dryMass, Litter.Totals$plotID) not going to run this one rn                 #From package FSA
#would report chi statistic , p value, and degress of freedom
```
...
```{r}
# Format as aov, + shows us ??? two different predictable variables, a lot of data 
...
Litter.groups #many associated with group d
