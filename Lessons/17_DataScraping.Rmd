...
* DOM parsing: we'll be using this today

We will be scraping today using DOM parsing. First, we need to install a tool on our web browser to be able to call the web text we need. The tool is called a Selector Gadget, which for Chrome can be found [here](https://chrome.google.com/webstore/detail/selectorgadget/mhjhnkcfbdhnjickkkdbjoemdmbfginb?hl=en). she considers this one the most helpful
...
```{r}
# Specify website to be scraped, tables with data in them with river quality assessment, nutrient related causes, lakes and reservoirs that are impaired too and estuaries too; can also just download the data there
url <- "https://www.epa.gov/nutrient-policy-data/waters-assessed-impaired-due-nutrient-related-causes"

# Reading the HTML code from the website; this is making a new object; have to specify parts of webpage, use selector gadget in chrome; green or yellow = what is selected; red and white = not selected; trial and error of selecting and deselecting
...
State <- webpage %>% html_nodes("table:nth-child(20) td:nth-child(1)") %>% html_text() #notice we have 23 character class pieces, shows up in values

Estuaries.Assessed.mi2 <- webpage %>% html_nodes("table:nth-child(20) td:nth-child(2)") %>% html_text() #read the page, then the specific nodes, then the text; they are all charcter strings which is good bc they all need to be the same
...
# Does R know that the numeric columns are numbers? we're changing everything from character to numeric
str(Estuary)
...
ggplot(Estuary, aes(x = Estuaries.Assessed.percent, y = Estuaries.Impaired.percent, 
  ...
  geom_label(aes(label = State), nudge_x = -5, nudge_y = -5,                  size = 3, alpha = 0.8) +  
  labs(x = "% Estuaries Assessed (%)", y = "% Estuaries Impaired (%)", fill = "% Impaired with TMDL")
#the only difference is that only 4 states are labeled in 2nd one; number not percent in second one

ggplot(Estuary, aes(x = Estuaries.Assessed.mi2, y = Estuaries.Impaired.mi2, 
                    fill = Estuaries.Impaired.percent.TMDL)) +
  geom_point(shape = 21, size = 2, alpha = 0.8) + 
  scale_fill_viridis_c(option = "inferno", begin = 0.2, end = 0.9, direction = -1) +
  geom_label_repel(data = subset(Estuary, State %in% c("Virginia", "Florida", "Louisiana", "Texas")), aes(label = State), nudge_x = -500, nudge_y = 200) + labs(x = expression("Estuary Area Assessed (mi"^"2"*")"), y = expression("Estuary Area Impaired (mi"^"2"*")"), fill = "% Impaired with TMDL")
