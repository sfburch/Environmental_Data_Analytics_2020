...
LTR.chem.physics <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv")
...
```{r}
#3 first should be processed, and second part should be raw; NA omit removes any row within dataframe na; na exclude will omit any na from specific column or set of columns
LTR.Wrangle <- LTR.chem.physics %>% 
  filter(daynum > 181 & daynum < 213) %>% 
  select(lakename, year4, daynum, depth, temperature_C) %>% 
  na.omit()

#4
TempAIC <- lm(data = LTR.Wrangle, temperature_C ~ year4 + daynum + depth)

step(TempAIC) #which has the lowest AIC
...
> Answer: The lowest AIC option (26016.31) includes the three explanatory variables: year4, daynum, and depth. It explains 74.17%.

6. Run an interaction effects ANCOVA to predict temperature based on depth and lakename from the same wrangled dataset.

```{r}
#6  can get here the r-quared and interactions

LakeANCOVA.interactions <- lm(data = LTR.Wrangle, temperature_C ~ depth * lakename)

summary(LakeANCOVA.interactions)
...
> Answer: Yes, there is a significant interaction between both depth and lakename. The variance is 78.6%.
...
TempGraph <- ggplot(LTR.Wrangle, aes(x = depth, y = temperature_C, color = lakename)) +
    geom_point(alpha = .5) +
...
> Answer: After comparing the two AICs, it became evident that the mixed effects model is a better option than the fixed effects model, as the mixed effects model has the lower AIC of 9038.575 vs. 9058.088.