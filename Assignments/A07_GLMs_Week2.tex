...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{LTR.chem.physics <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#3 first should be processed, and second part should be raw; NA omit removes any row within dataframe na; na exclude will omit any na from specific column or set of columns}
\NormalTok{LTR.Wrangle <-}\StringTok{ }\NormalTok{LTR.chem.physics }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(daynum }\OperatorTok{>}\StringTok{ }\DecValTok{181} \OperatorTok{&}\StringTok{ }\NormalTok{daynum }\OperatorTok{<}\StringTok{ }\DecValTok{213}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{select}\NormalTok{(lakename, year4, daynum, depth, temperature_C) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{na.omit}\NormalTok{()}

\CommentTok{#4}
\NormalTok{TempAIC <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ LTR.Wrangle, temperature_C }\OperatorTok{~}\StringTok{ }\NormalTok{year4 }\OperatorTok{+}\StringTok{ }\NormalTok{daynum }\OperatorTok{+}\StringTok{ }\NormalTok{depth)}

\KeywordTok{step}\NormalTok{(TempAIC) }\CommentTok{#which has the lowest AIC}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#6  can get here the r-quared and interactions}

\NormalTok{LakeANCOVA.interactions <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ LTR.Wrangle, temperature_C }\OperatorTok{~}\StringTok{ }\NormalTok{depth }\OperatorTok{*}\StringTok{ }\NormalTok{lakename)}

\KeywordTok{summary}\NormalTok{(LakeANCOVA.interactions)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{TempGraph <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(LTR.Wrangle, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ depth, }\DataTypeTok{y =}\NormalTok{ temperature_C, }\DataTypeTok{color =}\NormalTok{ lakename)) }\OperatorTok{+}
...
\KeywordTok{print}\NormalTok{(TempGraph)}
\end{Highlighting}
\end{Shaded}
